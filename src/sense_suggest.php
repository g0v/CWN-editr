...
//set_time_limit(0);
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($match[1])==6) {
				...
				if($results){
					$cwn_records = "<select id=\"source_list\" class=\"form-control input-lg\">";
					while ($row = $results->fetchArray()) {
						$cwn_records .= "<option value=\"".$row['sense_id']."\">(".$row['sense_id'].") ".$row['sense_def']."</option>";
					}
					$cwn_records .= "</select>";
				
					if ($cwn_records=="<select id=\"source_list\" class=\"form-control input-lg\"></select>"){
						$cwn_records = "<select id=\"source_list\" class=\"form-control input-lg\"><option value=\"nogood\">尚未定義 (not defined yet)</option></select>";
					}
				echo $cwn_records;
				}
